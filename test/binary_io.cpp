...
auto make<bool>(std::size_t w, std::size_t h) {
    ...
    static std::uniform_int_distribution<signed char> dis(limits::min(), limits::max());

    bitmap<bool> result(w, h);
    std::generate(result.begin(), result.end(), [] { return dis(gen) < 0; });
    return result;
}
...
TYPED_TEST_SUITE(endian_read_write_test, all_types, );

TYPED_TEST(endian_read_write_test, LittleRWTest) {
    using type = typename TestFixture::type;
    auto img = make<type>(9, 9);
    std::stringstream s;
    binary_write(img, s, std::endian::little);
    EXPECT_EQ(s.str().size(), header_size + img.point_count() * sizeof(type));
    auto img2 = binary_read<type>(s);
    EXPECT_EQ(img, img2);
}
...
TYPED_TEST(endian_read_write_test, BigRWTest) {
    ...
    auto img = make<type>(9, 9);
    std::stringstream s;
    binary_write(img, s, std::endian::big);
    EXPECT_EQ(s.str().size(), header_size + img.point_count() * sizeof(type));
    auto img2 = binary_read<type>(s);
    EXPECT_EQ(img, img2);
}
