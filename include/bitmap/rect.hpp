...
namespace bmp {
    ...
    public:
        ...
        /// \brief Set width and height
        constexpr void set_size(width_type const& width, height_type const& height) {
    ...
    constexpr bool operator==(rect<XT, WT, YT, HT> const& l, rect<XT, WT, YT, HT> const& r) {
    ...
    template <typename XT, typename WT, typename YT, typename HT>
    constexpr auto join(rect<XT, WT, YT, HT> const& l, rect<XT, WT, YT, HT> const& r) {
    ...
    constexpr bool contains(rect<XT, WT, YT, HT> const& rect, point<PXT, PYT> const& point) {
    ...
    template <typename XT, typename WT, typename YT, typename HT>
    constexpr bool
        contains(rect<XT, WT, YT, HT> const& reference, rect<XT, WT, YT, HT> const& test) {
        return test.left() >= reference.left() && test.top() >= reference.top()
            && test.right() <= reference.right() && test.bottom() <= reference.bottom();
    }
