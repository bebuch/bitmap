...
namespace bmp {
    ...
    enum class transform { rigth_rotate = 0x01, mirror_horizontal = 0x02, mirror_vectical = 0x04 };
    ...
    inline bitmap<decltype(std::devlval<ConvertFn>()(std::devlval<T>()))> bitmap_transform(
        bitmap<T> const& image,
        ::bmp::transform transform,
        ConvertFn&& converter = &no_convert) {
        using result_type = decltype(std::devlval<ConvertFn>()(std::devlval<T>()));
        using size_type = typename bitmap<result_type>::size_type;

        bool const rotation = (transform & simple_bitmap_transform::rigth_rotate) != 0;

        bool const mirror_h = (transform & simple_bitmap_transform::mirror_horizontal) != 0;
