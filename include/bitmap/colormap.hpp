...
namespace bmp::colormap {
    ...
    std::vector<T> make_uniform(
        ...
        T const max_value = std::is_floating_point<T>::value ? 1 : std::numeric_limits<T>::max()) {
        ...
        for(std::size_t vi = 0; vi < values; ++vi) {
            ...
            auto step_index_s = static_cast<std::size_t>(std::lround(step_index));
            ...
            if(s2 > s1) {
                colormap[vi] = static_cast<T>((s1 + fn.rise(scaled) * (s2 - s1)) * max_value);
            } else if(s2 < s1) {
                colormap[vi] = static_cast<T>((s2 + fn.fall(scaled) * (s1 - s2)) * max_value);
            } else {
