...
namespace bitmap::detail{
	...
	enum class binary_io_flags: std::uint8_t{
		is_unsigned       = 0x00,
		is_signed         = 0x01,
	...
	constexpr std::uint8_t binary_io_flags_v = static_cast< std::uint8_t >(
		std::is_same_v< T, bool > ? binary_io_flags::is_bool :
		std::is_floating_point_v< T > ? binary_io_flags::is_floating_point :
		std::is_signed_v< T > ? binary_io_flags::is_signed :
			binary_io_flags::is_unsigned) |
		static_cast< std::uint8_t >(
			boost::endian::order::native == boost::endian::order::little
				? binary_io_flags::is_little_endian
				: binary_io_flags::is_big_endian);
